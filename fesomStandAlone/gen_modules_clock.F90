module g_clock
  ...
  character(4)             :: cfyearnew   !year as character string      
...
contains
  ...
  subroutine clock_init
    ...
    if (wind_data_source(1:6).eq.'HadCM3'.or.wind_data_source(1:7).eq.'HadGem2') then
     if (mype .eq. 0) write(*,*)'use 360 days per year, ignore leap years'
     ...
     if (mype .eq. 0) write(*,*)'use 365 days per year, consider leap years'
