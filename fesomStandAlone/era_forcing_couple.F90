...
!  integer, parameter        		:: nci=96, ncj=48     ! mpeh5c grid
  ...
  if (wind_data_source(1:10).eq.'ERAINT_20C') then
   if (.FALSE.) then   ! VH forcing manip runs     
...
!    write(*,*) yearnew,daynew,'pre-2016 init itime=',itime, ForcingDataPath
  ...
  do i=1,2       ! split reading commands due to differing filename   VH
...
!   write(*,*)'init wind end of do loop , now i=',i
  ...
  ! else if (yearnew.ge.2016 .and. yearnew .le. 2018) then 
   else if (.TRUE.) then    ! VH 
    itime= daynew
    if (yearnew.ge.1979 .and. yearnew .le. 2017) then
      cfyearnew=cyearnew
    else if (yearnew.ge.2018) then ! .and. yearnew .le. 2200) then    ! für fortlaufenden Run mit wiederholendem forcing VH
      if (MODULO((yearnew-2017),39) .eq. 0) then
        write(cfyearnew,'(i4)') 2017
      else
        write(cfyearnew,'(i4)') MODULO((yearnew-2017),39)+1978 
      end if
      if (mype==0) write(*,*) 'in year ', cyearnew, ' read forcing ',cfyearnew  
    end if
    ForcingDataPath=trim(ForcingDataPath)//'eraint_years/era_'//cfyearnew//'/'  ! path adjusted  VH 
    !write(*,*) yearnew,daynew,'post-2016 init itime=',itime, ForcingDataPath

  do i=1,2         ! split reading commands due to differing filename   VH
!   write(*,*)'init wind i,itime=',i,itime
   if (i.eq.1) then
#include "read_eraint_fyears_data1_1.incF90" ! 1 to 1_1 ha read itime ga chigau
!  write(*,*)'back from read_eraint_data1_1, now i=',i
   end if
!  write(*,*)'init wind between ifs. i=',i

   if (i.eq.2) then
#include "read_eraint_fyears_data2.incF90"
   end if
!   write(*,*)'init wind after ifs. i=',i
...
if (wind_data_source(1:10).eq.'ERAINT_20C') then
 !if (yearnew.ge.1979.and.yearnew.lt.2016) then   ! RTnew 07.08.2018
 if (.FALSE.) then   ! RTnew 07.08.2018
  itime=(yearnew-1979)*365 + int(float(yearnew-1977)/4.) + daynew  
!  write(*,*) yearnew,daynew,'pre-2016 readnew itime=',itime, ForcingDataPath
 
     ! split reading commands due to differing filename   VH
...
elseif (th_era.eq.0.25) then
! write(*,*) th_era, 'data3' 
#include "read_eraint_fyears_data3.incF90"
elseif (th_era.eq.0.5) then
! write(*,*) th_era, 'data4'
#include "read_eraint_fyears_data4.incF90"
elseif (th_era.eq.0.75) then
! write(*,*) th_era, 'data1'
#include "read_eraint_fyears_data1.incF90"
 endif
