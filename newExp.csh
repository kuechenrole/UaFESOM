...
cp -r $timmsmeshdir/$oldrunid/1979.00 $timmsmeshdir/$newrunid/.
cp -r $timmsmeshdir/$oldrunid/extended $timmsmeshdir/$newrunid/.
rm $masterdir/$newrunid/ice2oce*
rm $masterdir/$newrunid/oce2ice*
rm $masterdir/$newrunid/slurm*
rm $masterdir/$newrunid/$oldrunid.clock
...
ssh -i $HOME/.ssh/id_rsa_hlrn -t $fesomuser@$fesomhost cp $fesomresultsdir/start.clock $fesomresultsdir/$newrunid/$newrunid.clock
...
ssh -i $HOME/.ssh/id_rsa_hlrn -t $fesomuser@$fesomhost sed -i "s~MeshPath=.*~MeshPath=''\''$fesommeshdir/$newrunid/1979.00/'\'''~" $fesomrundir/$newrunid/namelist.config
ssh -i $HOME/.ssh/id_rsa_hlrn -t $fesomuser@$fesomhost cp -r $fesommeshdir/$oldrunid/1979.00 $fesommeshdir/$newrunid/.
ssh -i $HOME/.ssh/id_rsa_hlrn -t $fesomuser@$fesomhost mkdir $fesommeshdir/$newrunid/1979.00/dist
...
ssh -i $HOME/.ssh/id_rsa_linsrv1-albedo -t $uauser@$uahost cp $uarstdir/$oldrunid/*ii* $uarstdir/$newrunid/.
