...
if ~UserVar.Slipperiness.ReadFromFile
    ...
    if isempty(FC)
        ...
        if isfile(UserVar.CFile)
            ...
            % create a FC file
            fprintf('DefineSlipperyDistribution: loading file: %-s ',CtrlVar.NameOfFileForSavingSlipperinessEstimate)
            %load('C-Estimate.mat','C','xC','yC')
            load(CtrlVar.NameOfFileForSavingSlipperinessEstimate,'C','xC','yC')
