...
!RT:
   if (spinup) then
     if (daynew.eq.1) then
      if (mype .eq. 0) write(*,*)'new year, write initial data into file'
      call output(1)
     endif
   else
...
!RT-
  ...
  do istep=1, nsteps   
     call clock
     if (spinup) call init_output !OR: not neede any longer, as we initialise at every
     !restart
...
!        write(*,*) 'uf max', maxval(abs(uf)), 's min', minval(tracer(:,2)), istep
  ...
  if (mype .eq. 0) write(*,*) 'Experiment '//runid//' successfully completed'
