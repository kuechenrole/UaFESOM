...
subroutine read_init_ts
  ...
  if (mype .eq. 0) write(*,*)'open ',trim(ClimateDataPath)//trim(OceClimaDataName)
 ...
 if (trim(OceClimaDataName).eq.'annual_woa01_ts.out'.or. trim(OceClimaDataName).eq.'WOA13_TS.txt') then   ! added RT RG4150 !LS
  if (mype .eq. 0) write(*,*)'read ',trim(OceClimaDataName)
  do i=1, num_lon_reg
     do j=1, num_lat_reg
 ...
 if (trim(OceClimaDataName).eq.'annual_woa01_ts.out'.or. trim(OceClimaDataName).eq.'WOA13_TS.txt') then   ! added RT RG4150 !LS
  ...
  if (mype .eq. 0) write(*,*)'read_init_ts completed'
