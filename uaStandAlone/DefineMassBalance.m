...
if isempty(Fab)
    ...
    wnetFes = double(ncread(UserVar.fesomMeltPath,'wnet'));
    %wnetFes = mean(double(wnetFes(:,2:end)),2);
    fprintf(' done \n')
    
    fprintf('loading coordinates and mask from folder: %-s ',UserVar.fesomMeshPath)
    fid=fopen([UserVar.fesomMeshPath,'nod2d.out'],'r');
    n2d=fscanf(fid,'%g',1);
    nodes=fscanf(fid, '%g', [4,n2d]);
    fclose(fid);
   
    fid=fopen([UserVar.fesomMeshPath,'cavity_flag_nod2d.out'],'r');
